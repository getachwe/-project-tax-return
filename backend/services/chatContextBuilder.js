...
/** טקסט ידע כללי — לא כולל נתוני משתמש */
const GUEST_GENERAL_KNOWLEDGE = `
מערכת "מס החזר" מאפשרת:
- העלאת טופס 106 (PDF) לחילוץ נתונים או מילוי ידני של השדות.
- חישוב החזר מס משוער לפי הנתונים שהוזנו.
- שמירת דוח PDF בהיסטוריה (למשתמשים מחוברים).
- שליחת סיכום למייל (למשתמשים מחוברים).

טופס 106 הוא טופס שנתי של הכנסות ומס מהמעסיק/לשכת המס.
המערכת אינה מחליפה ייעוץ מס או החלטת רשות המיסים; התוצאות מבוססות על הנתונים שהוזנו.

שאלות נפוצות:
- איך מתחילים: התחברות, ואז "העלאת מסמכים" או מילוי טופס.
- היסטוריה: תפריט "היסטוריה" מציג דוחות שמורים.
- אבטחה: נתונים מקושרים לחשבון המשתמש ואינם חשופים לאחרים.
`.trim();
